...
function displayDef(w){let s=String(typeof w==='string'?w:(w.aiDefinition||w.definition)||'').replace(/[\u007f]/g,' ').replace(/\[[^\]]*\]/g,'').replace(/^\s*[—-]?\s*(?:(?:adj|adv|n|v|a)\.|(?:adjective|noun|verb|adverb)\b)\s*/i,'').replace(/^\s*\([^)]*\)\s*/,'').replace(/^\s*[—-]\s*/,'').replace(/\s+/g,' ').trim();s=s.split(/\s+[—-]\s*(?:(?:adj|adv|n|v)\.|(?:adjective|noun|verb|adverb)\b)/i)[0].trim();s=s.replace(/\b(?:foll\.|usu\.|colloq\.|esp\.)\s*/gi,'').trim();
