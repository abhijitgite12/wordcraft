...
- **Vocal agent / orchestrator loop now truly moves the page (verified).** The tutor picks a real page action from the loop — `next`/`back`/`reveal`/`deep_dive`/`answer_option`/`load_word`/`test_category` — and the card updates accordingly. Every decision is logged server-side as `[orch] session=… -> action=…`, so you can watch the tutor actually navigating as it interacts.
- **Tutor can load any word & quiz on a category.** New tools `load_word` (study a specific word — e.g. "teach me the word elucidate") and `test_category` (quick quiz on GRE/SAT/Core/Academic). Reachable by voice AND the new top search bar.
- **Top-center search bar (new).** 🔍 in the header: type a word → Enter (or click result) makes that word the flashcard; a category keyword (GRE/SAT/Core/Academic) starts a 5-word quick quiz. Same code path the tutor uses.
- **Modern model lineup.** Primary `qwen/qwen3.8-flash`, fallbacks `deepseek/deepseek-v4-flash-0731` → `google/gemini-3.7-flash`. The old `:free` slots (gemma/glm) were returning "unavailable for free" so they were dropped. `max_tokens` raised 180→400 and the orch loop tolerates truncated JSON replies (no more local-fallback pop for long narrations).
- Cache-bust: `style.css?v=wordcraft25`, `app.js?v=wordcraft34`.
...
- **Human tutor narration:** proactive teaching voice that speaks the word on card change, teaches definition/example/synonyms on reveal, praises on correct, supports on wrong, guides deep dive — built from real card data (no canned filler). Engaged when voice is on.
- **About / version badge:** ⓘ button in header → popover with git commit, branch, deploy time, server-up time. Served by `GET /api/version`; uses Render's auto-injected `RENDER_GIT_COMMIT`/`RENDER_GIT_BRANCH`/`RENDER_DEPLOY_DATE` on prod.
